...
TEST_F(PerformanceTestV2, wrapperPerf_eq) {
  ...
  std::string value;
  encoder.Encode('r', record, key, value);
  EXPECT_EQ(encoder.GetCodecVersion(), dingodb::V2::CODEC_VERSION_V2);
  ...
  encoder.SetCodecVersion(dingodb::V2::CODEC_VERSION_V1);
  std::string key1;
  std::string value1;
  encoder.Encode('r', record_v1, key1, value1);
  EXPECT_EQ(encoder.GetCodecVersion(), dingodb::V2::CODEC_VERSION_V1);
...
TEST_F(PerformanceTestV2, perf) {
  ...
  dingodb::V2::RecordEncoderV2 encoder(1, schemas, 100);
