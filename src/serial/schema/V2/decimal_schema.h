...
#ifndef DINGO_SERIAL_DECIMAL_SCHEMA_V2_H_
#define DINGO_SERIAL_DECIMAL_SCHEMA_V2_H_
...
template <>
class DingoSchema<DecimalString> : public BaseSchema {
 public:
  Type GetType() override { return kDecimal; }
  int GetLengthForKey() override;
  int GetLengthForValue() override;

  BaseSchemaPtr Clone() override {
    return std::make_shared<DingoSchema<DecimalString>>();
  }
