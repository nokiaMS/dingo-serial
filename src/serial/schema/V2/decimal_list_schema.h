...
#ifndef DINGO_SERIAL_DECIMAL_LIST_SCHEMA_V2_H_
#define DINGO_SERIAL_DECIMAL_LIST_SCHEMA_V2_H_
...
template <>
class DingoSchema<std::vector<DecimalString>> : public BaseSchema {
 public:
  Type GetType() override { return kDecimalList; }
  int GetLengthForKey() override;
  int GetLengthForValue() override;

  BaseSchemaPtr Clone() override {
    return std::make_shared<DingoSchema<std::vector<DecimalString>>>();
  }
 ...
 private:
  static int EncodeDecimalListNotComparable(const std::vector<std::string>& data,
                                           Buf& buf);
  static void DecodeDecimalListNotComparable(Buf& buf,
